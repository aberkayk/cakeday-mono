...
export function CakeSelector({
  ...
  return (
    <div className="space-y-6">
      {/* Cake type selection */}
      <div>
        <p className="text-sm font-semibold text-on-surface mb-3">Pasta Türü</p>
        {cakeTypes.length === 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="rounded-2xl border-2 border-outline-variant bg-surface-container-low h-28 animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {cakeTypes.map((cake) => (
              ...
              >
                ...
                <div className="h-14 w-full bg-gradient-to-br from-primary-fixed to-primary-fixed/60 rounded-xl mb-2.5 flex items-center justify-center text-2xl">
                  🎂
                </div>
                <p className="text-sm font-semibold text-on-surface truncate">{cake.name}</p>
                {(cake.is_gluten_free || cake.is_vegan) && (
                  <div className="flex gap-1 mt-1.5 flex-wrap">
                    {cake.is_gluten_free && (
                      <Badge className="text-[10px] px-1.5 py-0 rounded-full bg-surface-container text-on-surface-variant border-0 font-semibold">GF</Badge>
                    )}
                    {cake.is_vegan && (
      ...
      {/* Size selection */}
      <div>
        <p className="text-sm font-semibold text-on-surface mb-3">Pasta Boyutu</p>
          ...
          {Object.entries(CAKE_SIZE_LABELS).map(([key, label]) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-on-surface-variant mt-0.5">{config.desc}</p>
