...
export default function BakeriesPage() {
  ...
  const fetchBakeries = async () => {
    setIsLoading(true);
    try {
      const res = await adminApi.bakeries({ page, pageSize, search: search || undefined });
      setBakeries(res.data);
      setTotalCount(res.meta.totalCount);
    } catch { } finally { setIsLoading(false); }
  };

  useEffect(() => { fetchBakeries(); }, [page, search]);
                  ...
                  <TableRow key={bakery.id} className="hover:bg-muted/20">
                    ...
                    <TableCell>
                      <Button variant="ghost" size="icon" className="h-8 w-8" asChild>
                        <Link href={`/dashboard/bakeries/${bakery.id}`}><ChevronRight className="h-4 w-4" /></Link>
                      </Button>
                    </TableCell>
                  </TableRow>
                ))
