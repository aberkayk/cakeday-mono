...
export class EmailService {
  ...
  private async send({
    ...
    try {
      const { data, error } = await this.resend.emails.send({
        from: `CakeDay <${process.env.RESEND_FROM_EMAIL ?? 'noreply@cakeday.com.tr'}>`,
        to: toList,
        subject,
        html,
        text,
  ...
  async sendVerificationEmail(email: string, name: string, token: string) {
    const verificationUrl = `${process.env.NEXT_PUBLIC_URL ?? 'http://localhost:3000'}/verify-email?token=${token}`;
