      ...
      <DialogContent className="max-w-md rounded-2xl">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-5">
          ...
          <div className="space-y-4">
            <p className="text-xs font-semibold text-muted uppercase tracking-wider">Kural Bilgileri</p>

            <div className="space-y-1.5">
              <Label htmlFor="name" className="text-sm font-medium text-foreground">Kural Adı *</Label>
              <Input
                id="name"
            ...
            <div className="space-y-1.5">
              <Label className="text-sm font-medium text-foreground">Kural Tipi *</Label>
              <Select
          ...
          {ruleType === "round_birthdays" && (
            <div className="space-y-1.5 rounded-xl bg-tertiary-container/20 border border-tertiary/20 p-4">
              <Label htmlFor="milestone_ages" className="text-sm font-medium text-foreground">Yuvarlak Yaşlar</Label>
              <Input
                id="milestone_ages"
                placeholder="30, 40, 50"
                {...register("milestone_ages")}
                className="rounded-xl border-border-soft focus:border-tertiary bg-background"
              />
              <p className="text-xs text-muted">Virgülle ayırın. Örn: 30, 40, 50</p>
            </div>
          )}

          {ruleType === "work_anniversary" && (
            <div className="space-y-1.5 rounded-xl bg-secondary-container/20 border border-secondary/20 p-4">
              <Label htmlFor="anniversary_years" className="text-sm font-medium text-foreground">Yıldönümü Yılları</Label>
              <Input
                id="anniversary_years"
                placeholder="1, 3, 5, 10"
                {...register("anniversary_years")}
                className="rounded-xl border-border-soft focus:border-secondary bg-background"
              />
              <p className="text-xs text-muted">Virgülle ayırın. Örn: 1, 3, 5, 10</p>
            </div>
          )}

          {/* Cake config */}
          <div className="space-y-4 pt-1">
            <p className="text-xs font-semibold text-muted uppercase tracking-wider">Pasta Ayarları</p>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="custom_text_template" className="text-sm font-medium text-foreground">Pasta Mesajı Şablonu</Label>
              <Textarea
              ...
              />
              <p className="text-xs text-muted">{"{{ad}} çalışan adıyla otomatik değiştirilir."}</p>
            </div>
          </div>
